...
class VNAStream():
    ...
    def init_params(self):
        ...
        self.s_data = []
        self.timestamp = []
        self.keydown_timestamps = []
        self.keyup_timestamps = []
    ...
    def on_release(self, event):
        ...
        if mkey == " ":
            self.space_pressed = False
            if self.isRecord and getattr(self, 'record_start_time', None) is not None:
                keyup_timestamp = time.time() - self.record_start_time
                self.keyup_timestamps.append(keyup_timestamp)
                print(f"Space Up at {keyup_timestamp}")
    ...
    def on_press(self, event, force_key=None):
        ...
        # time data
            ...
            if self.mode == "start":
                print("start classification mode")
                self.predictions = []
            ...
            # Track spacebar press times to indicate ground truth sensor presses
            if not getattr(self, 'space_pressed', False):
                self.space_pressed = True
                key_timestamp = time.time() - self.record_start_time
                if self.isRecord and getattr(self, 'record_start_time', None) is not None:
                    self.keydown_timestamps.append(key_timestamp)
                    print(f"Space Down at {key_timestamp}")
        elif mkey == "o":
    ...
    def save_data(self):
        ...
        np.savez(filename+'.npz',
                ...
                s21=s21,
                ts=self.timestamp,
                keydowns=self.keydown_timestamps,
                keyups=self.keyup_timestamps,
        ...
        if self.isRecord:
            ...
            if len(self.s_data) % self.max_port == self.port_number:
                ...
                if getattr(self, 'record_start_time', None) is not None:
                    self.timestamp.append(self.ts - self.record_start_time)
...
if __name__ == '__main__':
    ...
    parser.add_option("--out_dir", dest="out_dir", default="data/",
                      help="output directory to save .npz files", metavar="OUTDIR")
